...
app.post('/api/polls', authenticateUser, async (req, res) => {
  try {
    ...
    const shareLink = `${process.env.FRONTEND_URL || 'https://livepoll-4u85.onrender.com'}/simple-vote.html#${pollId}`;
    const userId = req.user.id || req.user._id;
...
app.listen(PORT, () => {
  ...
  console.log(`📊 Storage: In-Memory (No MongoDB Dependencies)`);
